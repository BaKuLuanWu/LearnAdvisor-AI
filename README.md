...
|       └── result.py                    # 响应体封装
├── config/
|   ├── prompt/                          # 意图识别、各意图角色设定、对话摘要等提示词
|   ├── static/                          # 预定义标签、问题推荐模板等静态变量
|   ├── settings.py                      # 全局日志、跨域设置
|   └── sql_db.py                        # 关系型数据库配置
├── controller/                          # 控制层
├── dao/                                 # 持久层，解耦业务层与数据库的交互
├── infra/
|   ├── sql_db/  
|   |   ├── entity/                      # 存储实体类
|   |   └── repo/                        # 数据表访问
...
|   ├── chat_service/                    # 对话聊天模块
|   |   └── ro_service/                  # 工作流编排
|   |       ├── context_query_engine.py  # 上文查询判断引擎
|   |       ├── dialog_state.py          # 对话状态机
|   |       ├── intent_recognizer.py     # 意图识别器
|   |       ├── reply_module.py          # 模型回复
|   |       └── task_router.py           # 任务路由
|   ├── conv_memory_manage_service/      # 上下文管理模块
|   |   └── schedule_task.py             # 定时任务
|   ├── intent_service/                  # 意图识别模块
